...
export const criarNotificacao = async (
  ...
  try {
    ...
    // O enfileiramento do email NÃO pode derrubar nem ATRASAR o fluxo: se o
    // Redis estiver indisponível/mal configurado, a notificação já foi salva e o
    // envio direto (feito por provaController/feedbackController) ainda deve
    // acontecer. Por isso o `add` é "fire-and-forget" (sem await): mesmo que a
    // conexão com o Redis fique pendurada, criarNotificacao retorna na hora.
